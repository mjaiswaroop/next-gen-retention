import logging
from database import SessionLocal, engine, Base, active_tenant_id
...
def seed():
    ...
    try:
        ...
        if not admin_exists:
            ...
            m = db.query(Merchant).execution_options(skip_tenant_check=True).filter(Merchant.name == 'Retention Core Corp').first()
            if not m:
                m = Merchant(name='Retention Core Corp', api_key=secrets.token_urlsafe(32), is_active=True)
                db.add(m)
                db.commit()
            
            # Set active tenant context so User creation passes multi-tenant database check
